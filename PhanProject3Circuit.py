...
from SolarGenerator import SolarGenerator
...
class Circuit:
    ...
    def add_generator(self, name, bus, Xsub, X2, X0, Zg, grounded,voltage_setpoint, mw_setpoint):
        self.generators[name] = Generator(name, bus, Xsub, X2, X0, Zg, grounded,voltage_setpoint, mw_setpoint)

    def add_load(self, name, bus, real_power, reactive_power):
        self.loads[name] = Load(name, bus, real_power, reactive_power)

    def install_solar(self, id, bus, capacity_kw, irradiance_factor, efficiency_factor):
        solar_unit = SolarGenerator(id, bus, capacity_kw, irradiance_factor, efficiency_factor)
        generated_power = solar_unit.output_power_kw()
        # treat solar injection as negative real load (generation subtracts demand)
        self.add_load(id, bus, -generated_power, 0.0)
